...
#             print(weights)
    ...
    for i in range(0,n):
        ...
        np.save(base_model_name[i]+" "+data_name+'_concept&noise.npy',accuracy)
        for index, value in enumerate(weights[:,i]):
            if index%100 == 0:
                y_weight.append(value)
        weight=np.array(weight)
        np.save(base_model_name[i]+" "+data_name+'_concept&noise_weight.npy',weight)
    ...
    np.save('OEC'+" "+data_name+'_concept&noise.npy',y_total)

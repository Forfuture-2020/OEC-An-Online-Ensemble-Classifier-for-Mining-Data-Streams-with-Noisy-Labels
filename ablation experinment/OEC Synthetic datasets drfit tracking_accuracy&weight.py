...
def model_init():
    ...
    demo_loss = loss.Ramp(parameter = -1.5, policy = "static") 
    # Construct classifier
    clfnolca_poly = NOLCA(demo_kernel, loss = demo_loss)
    clfnolca_rbf = NOLCA(demo_kernel_3,loss = demo_loss)
    model = [clfnolca_poly,clfnolca_rbf]
    model_name = ['ramp-SVM(Poly)','ramp-SVM(RBF)']
...
def build_model(x,y,noise_x,yita_l):
    yita_set = []
    ...
    for i in range(0,n):
        ...
        np.save(base_model_name[i]+data_name+'_concept&noise_concept tracking.npy',accuracy)
        for index, value in enumerate(weights[:,i]):
            if index%100 == 0:
                y_weight.append(value)
        weight=np.array(weight)
        np.save(base_model_name[i]+data_name+'_concept&noise_concept tracking_weight.npy',weight)
    ...
    np.save('OEC'+data_name+'_concept&noise_concept tracking.npy',y_total)
